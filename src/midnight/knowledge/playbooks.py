...
PLAYBOOKS: tuple[Playbook, ...] = (
    ...
    Playbook(
        "pwn-oversized-stack-write",
        "pwn",
        ("oversized write", "fixed stack", "arr_ptr.offset", "read_exact", "split_at", "unsafe rust", "size mismatch"),
        "Turn a source-confirmed oversized stack write into controlled return flow while satisfying post-copy checks.",
        (
        ...
        (
            "Do not infer a format string merely because leaked bytes resemble text; require an uncontrolled printf-family sink.",
            "A crash in a language panic path may prove corruption but still occur before the overwritten return address is consumed.",
            "Line-oriented APIs may stop on newline while raw reads still accept embedded NUL bytes.",
        ),
        "The exact input passes post-copy validation, reaches a controlled instruction pointer locally, and produces new target evidence.",
    ),
    Playbook(
    ...
    Playbook(
        ...
        (
            ...
            "Consume the program's post-input banner first, then parse the leak with recvline().rstrip() and ljust(8, b'\\0'); never use recv(8) across mixed banner and leak output.",
            "Compute libc base and assert page alignment. Stage two: add a ret gadget when needed, call system('/bin/sh') or an ORW chain, then exercise the target with run_exploit.",
        ),
        (
            ...
            "Restricted targets may make /dev/null unwritable; start shell validation with a plain command such as cat /flag before adding redirections.",
        ),
    ...
    Playbook(
        ...
        (
            "Fetch the root HTML and linked JavaScript before guessing routes; record the exact XML endpoint, method, and content type used by the client.",
            "Locate the parser library, entity settings, and where parsed fields enter the response or a later request.",
            ...
            "Extract candidates with the configured generic flag regex rather than a hard-coded lowercase prefix, and preserve the exact request and response.",
        ),
        (
    ...
    Playbook(
        ...
        (
            ...
            "Pickle has no GETATTR or GETITEM opcode; GET/BINGET retrieve memo slots, not mapping keys. Use a bound mapping method and REDUCE.",
        ),
