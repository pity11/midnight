...
def make_run_helper(*, base_state: CTFState, record_flag: Callable[[str], None]) -> Callable[..., object]:
    """Build the run_helper callable closed over the current challenge state."""
    cfg = get_config()

    async def run_helper(*, target_type: str, subtask: str, env: CTFEnvironment, depth: int, stack: list[str]) -> str:
        ...
        helper_state: CTFState = dict(base_state)  # shallow copy
        helper_state["escalation_depth"] = depth
        helper_state["escalation_stack"] = stack
        helper_state["challenge_type"] = target_type  # type: ignore[assignment]
        ...
        transcript = "\n".join(
            getattr(m, "content", "") or "" for m in msgs
            if isinstance(getattr(m, "content", ""), str)
        )
        for f in extract_flags(transcript, flag_format=(base_state.get("challenge") or {}).get("flag_format")):
            record_flag(f)
