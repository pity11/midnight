...
脱敏报告使用 `model_transport_failures` 累计已恢复的超时、连接和限流故障；即使后续尝试解题成功，该指标也不会丢失。
不可变 benchmark bundle 的 manifest 分类属于可信元数据，会直接路由到对应专家而不再调用分类模型；实时比赛平台提供的普通 hint 仍保留模型分类与降级路径。
Pwn 整题最多调用 `read_file` 12 次、`write_file` 6 次。首次生成 solver 后，只有新读取范围、结构化证据或新的 exploit 执行记录才能支持改写，避免弱模型在没有新事实时循环改文件。
带本地附件的 Pwn 必须先让同一脚本哈希在 local 模式断言控制效果并输出 `[MIDNIGHT_LOCAL_CONTROL_OK]`，之后才允许进入 target 模式；只有远程端点、没有附件的题保持兼容并跳过此门控。
